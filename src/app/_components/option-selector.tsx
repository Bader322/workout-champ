...
import styles from './_styles/option-selector.module.css';
...
const OptionSelector: React.FC<OptionSelectorProps> = ({ options , handler } ) => {
  const selection = useSelector((state: any) => state.skillLiftSelection);
  const dispatch = useDispatch();

  return (
    <Listbox value={selection} onChange={(e) => dispatch(handler(e))}>
      <div className='relative mt-2 min-w-[200px]'>
        <ListboxButton className='grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6'>
          <span className='col-start-1 row-start-1 flex items-center gap-3 pr-6'>
            <span className='block truncate'>{selection.name}</span>
          </span>
          <ChevronUpDownIcon
            aria-hidden='true'
        ...
        >
          {options.map((option) => (
            <ListboxOption
              key={option.id}
              value={option}
              className={`${styles.selection} group relative cursor-default py-2 pr-9 pl-3 text-gray-900 select-none data-focus:bg-indigo-600 data-focus:text-white data-focus:outline-hidden`}
            >
